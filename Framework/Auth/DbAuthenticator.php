...
class DbAuthenticator extends SessionAuthenticator
{
    ...
    protected function authenticate(string $username, string $password): ?IIdentity
    {
        ...
        $stmt = $db->prepare('SELECT ID_pouzivatela AS id, meno, priezvisko, email, heslo, admin, zabehnute_kilometre, vypite_piva, pohlavie FROM Pouzivatelia WHERE email = ? LIMIT 1');
        ...
        if ($hash !== '' && password_verify($password, $hash)) {
            ...
            $gender = isset($row['pohlavie']) ? $row['pohlavie'] : 'M';
            return new DbUser((int)$row['id'], $row['meno'] ?? '', $row['priezvisko'] ?? '', $row['email'] ?? '', $admin, $kilometres, $beers, $gender);
        }
